...
class Caption_Generator():
    ...
    def init_weight(self, dim_in, dim_out, name=None, stddev=1.0):
        return tf.Variable(tf.random.truncated_normal([dim_in, dim_out], stddev=stddev/math.sqrt(float(dim_in))), name=name)
    ...
    def build_model(self):
        ...
        for ind in range(self.n_lstm_steps):
            ...
            onehot_labels = tfc.sparse_to_dense( concated, tf.stack([self.batch_size, self.n_words]), 1.0, 0.0)
...
def train(pretrained_model_path=pretrained_model_path): # 전에 학습하던게 있으면 초기값 설정.
    ...
    train_op = tfc.train.AdamOptimizer(learning_rate).minimize(loss)
